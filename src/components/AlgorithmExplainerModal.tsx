...
const ALGO_INFO: Record<string, AlgoInfo> = {
  ...
  DIJKSTRA: {
    ...
    emoji: "🗺️",
    tagline: "Menor caminho a partir da origem, com destino opcional. Funciona em grafos direcionados, sem pesos negativos.",
    analogy:
    ...
    howItWorks: [
      "Inicialize dist[v] = ∞ e pred[v] = ∅ para todos os vértices.",
      "Coloque o nó inicial no conjunto S de explorados e faça dist[s] = 0.",
      "Em cada iteração, olhe a corte(S) e selecione a aresta (v, w) que minimiza dist[v] + d(v,w).",
      "Atualize dist[w] e pred[w], então adicione w ao conjunto S.",
      "Repita até alcançar o destino, se ele tiver sido definido, ou até não existir mais aresta ligando S ao restante do grafo.",
    ],
    complexity: {
      time: "O(V · E)",
      space: "O(V)",
      timeNote: "Nesta implementação didática, o corte é reavaliado a cada passo. Com fila de prioridade, seria O((V+E) log V).",
      spaceNote: "Armazena distâncias e predecessores para cada vértice.",
    },
    whatToWatch: [
      { color: "bg-slate-700", label: "Vértice não explorado" },
      { color: "bg-[#3aebb9]", label: "Vértice explorado" },
      { color: "bg-blue-500", label: "Aresta do corte" },
      { color: "bg-amber-400", label: "Aresta selecionada" },
      { color: "bg-[#3aebb9]", label: "Aresta do caminho" },
    ],
    tip: "Na sidebar, você pode escolher entre definir um destino específico ou calcular as distâncias mínimas da origem para todos os vértices. Se houver peso negativo, use Bellman-Ford.",
  },
  BELLMAN_FORD: {
    name: "Bellman-Ford",
    emoji: "🔁",
